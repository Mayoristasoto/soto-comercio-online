-- 1) Ampliar tipos de solicitud para licencias LCT
ALTER TABLE public.solicitudes_generales
  DROP CONSTRAINT IF EXISTS solicitudes_generales_tipo_solicitud_check;

ALTER TABLE public.solicitudes_generales
  ADD CONSTRAINT solicitudes_generales_tipo_solicitud_check CHECK (
    tipo_solicitud = ANY (ARRAY[
      'dia_medico','adelanto_sueldo','permiso',
      'matrimonio','fallecimiento_familiar','nacimiento_hijo',
      'examen_estudiantil','maternidad','paternidad',
      'donacion_sangre','actividad_gremial','licencia_medica',
      'dia_estudio','justificacion_inasistencia'
    ])
  );

-- 2) RPC: Novedades para Liquidación de Sueldos
  ...
  LOOP
    ...
    WHILE v_d <= p_hasta LOOP
      ...
      IF v_turno.id IS NULL THEN
        -- sin turno asignado: salto
        v_d := v_d + 1;
        CONTINUE;
      END IF;

      -- Resolver horario del día
      ...
      ELSE
        -- día no laborable para este empleado
        v_d := v_d + 1;
        CONTINUE;
      END IF;
      ...
      IF v_feriado.id IS NOT NULL THEN
        -- Si existen asignaciones para este feriado, solo aplica si el empleado está incluido
      ...
      -- 2) Vacaciones aprobadas/gozadas
      ...
      IF v_estado IS NULL THEN
        ...
        IF v_aus.id IS NOT NULL THEN
          v_estado := 'LIC_MEDICA';
          v_detalle := COALESCE(v_aus.tipo_enfermedad, 'Licencia médica');
        END IF;
      END IF;

      -- 4) Solicitud general aprobada que cubra el día
      IF v_estado IS NULL THEN
        SELECT id, tipo_solicitud, motivo INTO v_sol
        FROM public.solicitudes_generales
        WHERE empleado_id = v_emp.id
          AND estado = 'aprobada'
          AND fecha_inicio <= v_d
          AND (fecha_fin IS NULL OR fecha_fin >= v_d)
        ORDER BY created_at DESC
        LIMIT 1;
        IF v_sol.id IS NOT NULL THEN
          v_estado := UPPER(v_sol.tipo_solicitud);
          v_detalle := COALESCE(v_sol.motivo, v_sol.tipo_solicitud);
        END IF;
      END IF;

      -- 5) Calcular horas trabajadas (entrada y salida válidas)
