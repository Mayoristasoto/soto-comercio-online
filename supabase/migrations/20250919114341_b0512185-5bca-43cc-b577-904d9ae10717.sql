...
-- Crear una columna de fecha para índice
ALTER TABLE public.fichajes ADD COLUMN fecha_fichaje date GENERATED ALWAYS AS (date(timestamp_real)) STORED;
...
-- Crear índices para rendimiento usando la columna generada
CREATE INDEX idx_fichajes_empleado_fecha ON public.fichajes(empleado_id, fecha_fichaje);
