-- Update the premio_tipo enum to have proper prize types
ALTER TYPE premio_tipo RENAME TO premio_tipo_old;

CREATE TYPE premio_tipo AS ENUM ('fisico', 'digital', 'experiencia', 'descuento', 'reconocimiento');

-- Update the column to use the new enum type (convert existing values)
ALTER TABLE public.premios 
ALTER COLUMN tipo TYPE premio_tipo USING 
  CASE 
    WHEN tipo::text = 'semanal' THEN 'fisico'::premio_tipo
    WHEN tipo::text = 'mensual' THEN 'digital'::premio_tipo  
    WHEN tipo::text = 'semestral' THEN 'experiencia'::premio_tipo
    WHEN tipo::text = 'anual' THEN 'reconocimiento'::premio_tipo
    ELSE 'fisico'::premio_tipo
  END;

-- Drop the old enum type
DROP TYPE premio_tipo_old;

-- Now insert example prizes
