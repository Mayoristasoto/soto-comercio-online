-- Función para generar PINs aleatorios para todos los empleados activos
-- Retorna los PINs generados (en texto plano, solo para este uso)
...
BEGIN
  ...
  CREATE TEMP TABLE IF NOT EXISTS pins_generados (
    empleado_id uuid,
    nombre text,
    apellido text,
    legajo text,
  ...
  LOOP
    ...
    VALUES (
      v_empleado.id,
      crypt(v_nuevo_pin, gen_salt('bf', 8)),
      true,
      v_admin_id,
      0
    )
    ...
    DO UPDATE SET 
      pin_hash = crypt(v_nuevo_pin, gen_salt('bf', 8)),
      activo = true,
      intentos_fallidos = 0,
