-- Fix the foreign key constraint error in authenticate_face_kiosk function
-- The audit log should reference the system user instead of auth.uid()

CREATE OR REPLACE FUNCTION public.authenticate_face_kiosk(
  p_face_descriptor double precision[], 
  p_threshold double precision DEFAULT NULL::double precision
)
...
BEGIN
  ...
  -- Obtener un usuario del sistema para auditoría (primer admin encontrado)
  SELECT user_id INTO system_user_id 
  FROM empleados 
  WHERE rol = 'admin_rrhh' AND activo = true AND user_id IS NOT NULL 
  LIMIT 1;
  ...
  IF best_match IS NOT NULL THEN
    -- Log del acceso para auditoría con usuario del sistema
