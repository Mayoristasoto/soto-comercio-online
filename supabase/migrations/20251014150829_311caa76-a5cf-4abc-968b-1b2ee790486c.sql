-- Corregir función kiosk_insert_fichaje para evitar múltiples entradas en el mismo día
...
BEGIN
  ...
  IF p_datos ? 'tipo' THEN
    ...
    -- VALIDACIÓN: Si intenta registrar una entrada y ya hay una entrada hoy, rechazar
    IF fichaje_tipo = 'entrada' AND ultimo_fichaje_hoy IS NOT NULL THEN
      RAISE EXCEPTION 'Ya existe un fichaje de entrada para el día de hoy';
    END IF;
