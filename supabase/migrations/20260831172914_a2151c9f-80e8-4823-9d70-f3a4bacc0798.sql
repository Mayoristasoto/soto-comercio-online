CREATE OR REPLACE FUNCTION public.get_novedades_liquidacion(p_desde date, p_hasta date, p_sucursales uuid[] DEFAULT NULL::uuid[], p_empleados uuid[] DEFAULT NULL::uuid[])
 RETURNS TABLE(empleado_id uuid, empleado_nombre text, empleado_apellido text, empleado_legajo text, sucursal_id uuid, sucursal_nombre text, fecha date, dia_semana integer, estado text, detalle text, hora_entrada_esperada time without time zone, hora_salida_esperada time without time zone, horas_esperadas numeric, horas_trabajadas numeric, turno_nombre text, feriado_nombre text, justificacion_id uuid)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  LOOP
    ...
    WHILE v_d <= p_hasta LOOP
      ...
      v_just_id := NULL; v_just_cat := NULL; v_just_ok := NULL; v_just_obs := NULL;
      ...
      IF v_estado IS NULL THEN
        IF v_first_in IS NOT NULL THEN
          v_estado := 'TRABAJADO';
        ELSE
          -- Justificaciones cargadas desde el informe gerencial (Resumen del mes)
          SELECT j.id, c.nombre, COALESCE(c.es_justificada, true), j.observacion
            INTO v_just_id, v_just_cat, v_just_ok, v_just_obs
          FROM public.justificaciones_asistencia j
          LEFT JOIN public.categorias_justificacion_asistencia c ON c.id = j.categoria_id
          WHERE j.empleado_id = v_emp.id
            AND j.fecha_evento = v_d
            AND j.tipo_evento = 'ausencia'
          ORDER BY j.created_at DESC
          LIMIT 1;

          IF v_just_id IS NOT NULL AND COALESCE(v_just_ok, true) THEN
            v_estado := 'AUSENCIA_JUSTIFICADA';
            v_detalle := COALESCE(v_just_cat, 'Ausencia justificada')
                         || COALESCE(' — ' || v_just_obs, '');
          ELSIF v_just_id IS NOT NULL THEN
            v_estado := 'NO_FICHADA';
            v_detalle := 'No justificada: ' || COALESCE(v_just_cat, 'sin motivo')
                         || COALESCE(' — ' || v_just_obs, '');
          ELSE
            v_estado := 'NO_FICHADA';
      ...
      justificacion_id := COALESCE(v_just_id, v_sol_id, v_aus_id, v_vac_id);
      RETURN NEXT;
