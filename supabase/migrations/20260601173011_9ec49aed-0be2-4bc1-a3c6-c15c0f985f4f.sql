  ...
  LOOP
    ...
    WHILE v_d <= p_hasta LOOP
      ...
      IF v_feriado.id IS NOT NULL THEN
        IF EXISTS (SELECT 1 FROM public.feriado_empleados_asignados fea WHERE fea.feriado_id = v_feriado.id) THEN
          IF EXISTS (
            SELECT 1 FROM public.feriado_empleados_asignados fea
            WHERE fea.feriado_id = v_feriado.id AND fea.empleado_id = v_emp.id
          ) THEN
            v_estado := 'FERIADO';
      ...
      IF v_estado IS NULL THEN
        SELECT am.id, am.tipo_enfermedad INTO v_aus
      ...
      IF v_estado IS NULL THEN
        SELECT sg.id, sg.tipo_solicitud, sg.descripcion INTO v_sol
