...
DECLARE
  ...
  v_dow int := EXTRACT(ISODOW FROM v_hoy)::int; -- 1=lun..7=dom
  v_is_admin boolean := has_role(auth.uid(), 'admin_rrhh') OR has_role(auth.uid(), 'admin');
...
BEGIN
  ...
  vac AS (
    SELECT DISTINCT sv.empleado_id
    FROM solicitudes_vacaciones sv
    WHERE sv.estado = 'aprobada'
      AND v_hoy BETWEEN sv.fecha_inicio AND sv.fecha_fin
  ),
  lic AS (
    SELECT DISTINCT am.empleado_id
    FROM ausencias_medicas am
  ...
  SELECT
    ...
    CASE
      ...
      ELSE 'franco'
    END AS estado,
    fe.primer_entrada AS hora_entrada,
    fe.ultimo_ts AS hora_evento
  FROM base b
...
GRANT EXECUTE ON FUNCTION public.dashboard_estado_personal_hoy() TO authenticated;
