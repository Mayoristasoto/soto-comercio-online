...
CREATE TABLE public.fichado_ubicaciones (
  ...
  radio_metros integer DEFAULT 100, -- Radio de la geocerca en metros
  ip_whitelist text[], -- IPs permitidas para fichaje
...
CREATE TABLE public.fichajes (
  ...
  timestamp_aplicado timestamp with time zone, -- Después de aplicar redondeos
  ...
  longitud numeric(11,8),
  ip_address inet,
  confianza_facial numeric(3,2), -- 0.00 a 1.00
...
CREATE INDEX idx_fichajes_empleado_fecha ON public.fichajes(empleado_id, date(timestamp_real));
...
CREATE INDEX idx_incidencias_empleado ON public.fichaje_incidencias(empleado_id, fecha_incidencia);
...
BEGIN
  IF TG_OP = 'DELETE' THEN
    INSERT INTO public.fichaje_auditoria (tabla_afectada, registro_id, accion, datos_anteriores)
    VALUES (TG_TABLE_NAME, OLD.id, TG_OP, row_to_json(OLD));
    RETURN OLD;
  ELSIF TG_OP = 'UPDATE' THEN
    INSERT INTO public.fichaje_auditoria (tabla_afectada, registro_id, accion, datos_anteriores, datos_nuevos)
    VALUES (TG_TABLE_NAME, NEW.id, TG_OP, row_to_json(OLD), row_to_json(NEW));
    RETURN NEW;
  ELSIF TG_OP = 'INSERT' THEN
    INSERT INTO public.fichaje_auditoria (tabla_afectada, registro_id, accion, datos_nuevos)
...
CREATE TRIGGER audit_fichajes AFTER INSERT OR UPDATE OR DELETE ON public.fichajes
  FOR EACH ROW EXECUTE FUNCTION public.audit_trigger_function();

CREATE TRIGGER audit_incidencias AFTER INSERT OR UPDATE OR DELETE ON public.fichaje_incidencias
  FOR EACH ROW EXECUTE FUNCTION public.audit_trigger_function();

CREATE TRIGGER audit_turnos AFTER INSERT OR UPDATE OR DELETE ON public.fichado_turnos
  FOR EACH ROW EXECUTE FUNCTION public.audit_trigger_function();