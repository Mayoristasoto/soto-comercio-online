-- Evitar ambigüedad con variables OUT (empleado_id) usando ON CONSTRAINT
...
BEGIN
  ...
  LOOP
    ...
    -- Insertar o actualizar PIN (usar constraint para evitar ambigüedad con OUT empleado_id)
    ...
    ON CONFLICT ON CONSTRAINT empleados_pin_empleado_unique
    DO UPDATE SET 
    ...
    -- Guardar resultado
    INSERT INTO pins_generados (t_empleado_id, t_nombre, t_apellido, t_legajo, t_pin_generado, t_ya_tenia_pin)
    VALUES (
      v_empleado.id,
      v_empleado.nombre,
      v_empleado.apellido,
  ...
  RETURN QUERY 
    SELECT 
      ...
      pg.t_pin_generado,
      pg.t_ya_tenia_pin
    FROM pins_generados pg
    ORDER BY pg.t_apellido, pg.t_nombre;
END;
$$;