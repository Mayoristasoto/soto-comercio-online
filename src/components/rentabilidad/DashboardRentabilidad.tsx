...
import { BarChart3, TrendingUp, TrendingDown, DollarSign, Users, FileDown } from "lucide-react";
import AlertasRentabilidad from "./AlertasRentabilidad";
import { exportRentabilidadPDF } from "@/utils/rentabilidadPDF";
...
export default function DashboardRentabilidad() {
  ...
  const { data: liveData } = useQuery({
    ...
    queryFn: async () => {
      ...
      gastos?.forEach((g) => { const b = byBranch.get(g.sucursal_id); if (b) b.gastos += Number(g.monto_total); });
      facturacion?.forEach((f) => { const b = byBranch.get(f.sucursal_id); if (b) { b.facturacion += Number(f.total); b.tickets += (f.cantidad_tickets || 0); } });
      horas?.forEach((h) => { const b = byBranch.get(h.sucursal_id); if (b) b.horas += (h.horas_normales || 0) + (h.horas_extra || 0); });
  ...
  const rows: any[] = snapshots && snapshots.length > 0 ? snapshots : liveData || [];
  ...
  const periodoLabel = selectedPeriodo ? `${selectedPeriodo.fecha_inicio} → ${selectedPeriodo.fecha_fin}` : "";
  ...
  const handleExportPDF = () => {
    exportRentabilidadPDF({
      ...
      rows: rows.map((r: any) => ({
        ...
        facturacion: r.facturacion_total || 0,
        gastos: r.total_gastos || 0,
        costoLaboral: r.total_costo_laboral || 0,
        resultado: r.resultado_operativo || 0,
        margen: r.margen_operativo || 0,
      })),
      totals,
    });
  };
    ...
    <Card>
      ...
      <CardContent className="space-y-6">
        ...
        {periodoId && (
          <>
            {/* Alerts */}
            <AlertasRentabilidad periodoId={periodoId} />
